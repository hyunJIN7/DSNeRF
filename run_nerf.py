...
def create_nerf(args):
    ...
    """
    embed_fn, input_ch = get_embedder(args.multires, args.i_embed)  # create PE , input_ch가 결국 PE 거친 output channel

    input_ch_views = 0 # direction part
    embeddirs_fn = None
    ...
    #fine model part
    model_fine = None
    if args.N_importance > 0:  #N_importance  : number of additional fine samples per ray
        if args.alpha_model_path is None:
...
def train():
    ...
    if args.dataset_type == 'colmap_llff':  #train_imgs, test_imgs, train_poses, test_poses, video_poses, depth_data, bds
    ...
    elif args.dataset_type == 'llff': #숙대 연수원
    ...
    if not args.colmap_depth:
        ...
        N_depth = int(args.N_rand * args.depth_rays_prop)  #TODO : ???
        N_rgb = args.N_rand - N_depth
    use_batching = not args.no_batching
    if use_batching:
        # For random ray batching
        print('get rays')
        rays = np.stack([get_rays_np(H, W, focal, p) for p in poses[:, :3, :4]], 0)  # [N, ro+rd = 2,  H, W, 3]
        if args.debug:
        ...
        if args.colmap_depth:
            ...
            for i in i_train:
                rays_depth = np.stack(get_rays_by_coord_np(H, W, focal, poses[i, :3, :4], depth_gts[i]['coord']),
                                      axis=0)  # 2 x N x 3   , (ray_d + ray+o , N, 3)
                ...
                else : weights = np.repeat(depth_gts[i]['error'][:, None, None], 3, axis=2)  # N x 1 x 3
