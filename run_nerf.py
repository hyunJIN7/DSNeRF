...
def train():
    ...
    if args.render_test:
        render_poses = np.array(poses[i_test]) #TODO :이거 활성화 시키면 된다!!!
    ...
    if args.render_only:
        ...
        with torch.no_grad():
            ...
            if args.render_test_ray:   #TODO : test pose check
        ...
        if args.colmap_depth:
            ...
            for i in i_train:
                ...
                if args.dataset_type == 'llff': 
                	...
                	weights = np.repeat(depth_gts[i]['error'][:, None, None], 3, axis=2)  # N x 1 x 
    ...
    for i in trange(start, N_iters):
        ...
        if i%args.i_testset==0 and i > 0 and len(i_test) > 0:
            ...
            images[i_test] = torch.Tensor(images[i_test]).to(device)
